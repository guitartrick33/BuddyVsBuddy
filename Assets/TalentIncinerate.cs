...
public class TalentIncinerate : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        incinerate = myPlayer.GetComponent<Incinerate>();
        isUnlocked = false;
    ...
    {
        //TIER1
        if (tier == TalentTier.TIER1 && type == TalentType.INCINERATE)
        {
            if (isUnlocked == false && pr.currentTalentPoints >= cost)
            {
                tm.unlockedTalents.Add(gameObject);
                incinerate.incinerateDuration += durationIncrease;
                isUnlocked = true;
        ...
        //TIER2
        else if (tier == TalentTier.TIER2 && type == TalentType.INCINERATE)
        {
            foreach (GameObject g in tm.unlockedTalents)
            {
                if (g.GetComponent<TalentIncinerate>() != null)
                {
                    if (g.GetComponent<TalentIncinerate>().tier == TalentTier.TIER1 &&
                        g.GetComponent<TalentIncinerate>().type == TalentType.INCINERATE)
                    {
                        prevTalent = g;
                    }  
                }
            }
            if (!prevTalent.GetComponent<TalentIncinerate>().isUnlocked)
            {
                textTimerBool = true;
                ttpErrors.text = "Learn the Tier 1 first!";
            }
            if (isUnlocked == false && pr.currentTalentPoints >= cost && prevTalent.GetComponent<TalentIncinerate>().isUnlocked)
            {
                tm.unlockedTalents.Add(gameObject);
                incinerate.incinerateDuration += durationIncrease;
                isUnlocked = true;
        ...
        //TIER3
        else if (tier == TalentTier.TIER3 && type == TalentType.INCINERATE)
        {
            foreach (GameObject g in tm.unlockedTalents)
            {
                if (g.GetComponent<TalentIncinerate>() != null)
                {
                    if (g.GetComponent<TalentIncinerate>().tier == TalentTier.TIER2 &&
                        g.GetComponent<TalentIncinerate>().type == TalentType.INCINERATE)
                    {
                        prevTalent = g;
                    }  
            ...
            {
                tm.unlockedTalents.Add(gameObject);
                incinerate.cooldownTime -= cdReduction;
                isUnlocked = true;
                pr.currentTalentPoints -= cost;
                unlockedOverlay.fillAmount = 0;
            }
